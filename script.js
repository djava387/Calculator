...
const funcOperator = (operator) => {
  ...
  if (!firstInput) {
    ...
    operatorValue = operator; //resets operator value
  } else {
    const calculation = calculate[operatorValue](firstInput, currentValue);
    calcDis.textContent = calculation; //updated display to be equal to results of calculation
    // console.log(firstInput, operatorValue, currentValue);
    // console.log("calculation", calculation);
    firstInput = calculation; //allows multiple values i.e resets result to first input
  }
...
inputButtons.forEach((inputButton) => {
  ...
  if (inputButton.classList.length === 0) {
    inputButton.addEventListener("click", () =>
      inputNumberValue(inputButton.value)
    );
  } else if (inputButton.classList.contains("operator")) {
    inputButton.addEventListener("click", () =>
      funcOperator(inputButton.value)
    );
